# =============================================================
# PitSimulator — Bus I2C compartido (dummy) (VARIANTE CONGELADA)
#
# Copia de components/_i2c_bus/_i2c_bus.hal.py del repo del simulador
# -- ver la nota grande de _pit_base.py (misma carpeta) sobre por qué
# existe esta copia y por qué no hay que dejarla divergir del
# original. ÚNICO cambio real respecto al .hal.py original: el
# import de acá abajo. Cuando este archivo se pega por paste-mode
# (firmware SIN este freeze), poll_input/register_line_handler/
# _settle ya están en el namespace global porque _base.hal.py se
# pegó ANTES, en el mismo exec(). Congelado como módulo real, en
# cambio, este archivo tiene su PROPIO namespace -- sin este import
# explícito, "register_line_handler(...)" de más abajo tiraría
# NameError apenas se importe este módulo (ver el import agregado
# justo antes de "class I2C").
#
...
# arriba. poll_input/register_line_handler/_settle viven en
# _pit_base (congelado, importado antes desde boot.py).
from _pit_base import poll_input, register_line_handler, _settle
